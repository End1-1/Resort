...
QWidget *FExpectedArrivals::lastElement()
{
    return ui->wd->lw();
}
...
void FExpectedArrivals::printArrival(WReportGrid *rg)
{
    ...
    while (fDD.nextRow()) {
        ...
        mainRows << r;
        r++;
        if (fDD.getString(14).length() > 0) {
            row = emptyRow;
            row[0] = fDD.getString(14).replace("\r", " ").replace("\n", " ");;
            rg->fModel->appendRow(row);
            addRows << r;
            r++;
        }
...
void FExpectedArrivals::printBoth(WReportGrid *rg)
{
    rg->fModel->clearData();
    QList<int> cols;
    cols << 100 //id
...
QString FExpectedArrivals::type()
{
    QString type;
    if (ui->rbEntry->isChecked()) {
        type = tr("Arrivals");
...
QString FExpectedArrivals::type2()
{
    QString type;
    if (ui->rbEntry->isChecked()) {
        type = tr("Entry");
