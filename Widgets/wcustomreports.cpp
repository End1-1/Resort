    ...
    ui(new Ui::WCustomReports)
{
    ...
    for (int i = 0; i < fDD.rowCount(); i++) {
        if (fDD.getValue(i, "f_groupAccess").toString() == "*") {
    ...
    for (int i = 0; i < fDD.rowCount(); i++) {
        QString groupName = fDD.getValue(i, "f_groupName").toString();
        for (int j = 0; j < ui->trMenu->topLevelItemCount(); j++) {
            if (ui->trMenu->topLevelItem(j)->text(0) == groupName) {
                ...
                item->setText(0, fDD.getValue(i, "f_name").toString());
                item->setData(0, Qt::UserRole, fDD.getValue(i, "f_id"));
                ui->trMenu->addTopLevelItem(item);
            }
        }
    }
