    ...
    ui(new Ui::WReportBuilder)
{
    ...
    while (fDD.nextRow()) {
        ...
        item->setData(Qt::UserRole, fDD.getString(0));
...
void WReportBuilder::on_lwNames_clicked(const QModelIndex &index)
{
    ...
    fDD.exec("select f_name, f_group, f_groupAccess, f_sql, f_widths, f_titles_en, "
               ...
               "from serv_reports where f_id=:f_id");
    if (fDD.rowCount() > 0) {
...
void WReportBuilder::on_btnSave_clicked()
{
    ...
    fDD[":f_menu"] = ui->cbMainMenu->currentIndex();
    fDD[":f_sql"] = ui->teSQL->toPlainText();
    fDD[":f_fields"] = fields;
    fDD[":f_titles_en"] = titles;
    fDD[":f_widths"] = widths;
    fDD[":f_sum"] = ui->leSum->text();
    fDD[":f_filter"] = filter;
    fDD[":f_subtotal"] = ui->leSubtotals->text();
    if (ui->leCode->asInt() == 0) {
        ui->leCode->setInt(fDD.insert("serv_reports"));
        QListWidgetItem *item = new QListWidgetItem(ui->lwNames);
        item->setText(ui->leName->text());
        item->setData(Qt::UserRole, ui->leCode->asInt());
        ui->lwNames->addItem(item);
    } else {
        fDD.update("serv_reports", where_id(ui->leCode->asInt()));
...
{
    if (ui->leCode->asInt() > 0) {
        if (message_confirm(tr("Reset colums?")) != QDialog::Accepted) {
            return;
        }
    }
    ui->lwCols->clear();
