...
void Hall::init()
{
    ...
    QString query = "select f_id, f_name, f_defaultMenu, f_itemForInvoice, f_receiptPrinter, "
            "f_vatDept, f_noVatDept, f_serviceItem, f_defaultSvc, f_prefix "
    ...
    for (QList<QList<QVariant> >::const_iterator it = fDbRows.begin(); it != fDbRows.end(); it++) {
        ...
        h->fDefaultSvcValue = it->value(8).toDouble();
        h->fPrefix = it->value(9).toString();
        fHallTable.append(h);
        fHallMap[h->fId] = h;
    }
...
void Hall::refresh()
{
    ...
    fDD.exec("select f_id, f_lockHost, f_order from r_table");
    for (QList<QList<QVariant> >::const_iterator it = fDbRows.begin(); it != fDbRows.end(); it++) {
        TableStruct *t = getTableById(it->at(0).toInt());
        if (!t) {
            continue;
