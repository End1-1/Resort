    ...
    ui(new Ui::FCardexSales)
{
    ...
    for (int i = 0; i < fDD.rowCount(); i++) {
        Field f;
        f.fTitle = fDD.getString(i, "f_title");
        f.fWidth = fDD.getInt(i, "f_width");
        f.fItems = fDD.getString(i, "f_items");
        fItems[fDD.getInt(i, "f_id")] = f;
    }
}
...
void FCardexSales::finalPrint(PPrintScene *ps, int top)
{
    ...
    for (QMap<int, Field> ::const_iterator it = fItems.begin(); it != fItems.end(); it++) {
        ...
        QString name;
        for (int i = 0; i < fDD.rowCount(); i++) {
            name += " " + fDD.getValue(i, "f_en").toString();
        }
        if (fDD.rowCount() > 0) {
