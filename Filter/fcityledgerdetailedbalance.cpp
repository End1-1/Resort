...
void FCityLedgerDetailedBalance::apply(WReportGrid *rg)
{
    ...
    for (int i = 0; i < rg->fModel->rowCount(); i++) {
        ...
        rg->fModel->setData(i, 5, rg->fModel->data(i, 3, Qt::EditRole).toDouble() - rg->fModel->data(i, 4,
                            Qt::EditRole).toDouble());
        if (i > 0) {
            rg->fModel->setData(i, 5, rg->fModel->data(i, 5, Qt::EditRole).toDouble() + rg->fModel->data(i - 1, 5,
                                Qt::EditRole).toDouble());
        }
    }
